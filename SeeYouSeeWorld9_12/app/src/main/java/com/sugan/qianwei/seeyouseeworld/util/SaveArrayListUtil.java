...
public class SaveArrayListUtil {
     ...
     * @param content 内容
     */
    public static void saveArrayList(Context context, ArrayList<String> searchList, String content, String spName) {
        if (searchList.size() == 0) {   //searchList里“无数据”
            //直接存
            searchList.add(content);
        } else {    //searchList里“有数据”
     ...
     * 读取sp里的数组
     */
    public static ArrayList<String> getSearchArrayList(Context context, String spName) {
        //先定位到文件
    ...
    public static void clearSearchArrayList(Context context, String spName){
        SharedPreferences.Editor editor = context.getSharedPreferences(
                spName, Context.MODE_PRIVATE).edit();
        editor.clear().apply();
    }
